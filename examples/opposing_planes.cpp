...
int main() {
    ShapePrimitive light(
        ...
        EmissiveMaterial<SolidColor>(Vec3(1.0, 1.0, 1.0))
    );
    ShapePrimitive left_plane(
        Plane(Pt3(0., 0., -10.), Vec3(-0.5, 0.5, 1.).normalize()),
        SpecularMaterial<SolidColor>(Vec3(201. / 255., 10. / 255., 64. / 255.), 0.1)
    );
    ShapePrimitive right_plane(
        Plane(Pt3(0., 0., -10.), Vec3(0.5, -0.5, 1.).normalize()),
        SpecularMaterial<SolidColor>(Vec3(0., 138. / 255., 216. / 255.), 0.1)
    );
    ShapePrimitive sphere1(
        Sphere(Pt3(1., 1., -5.), 0.8),
        RefractiveMaterial<SolidColor>(Vec3(1., 1., 1.), 1.2)
    );
